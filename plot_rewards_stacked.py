...
data_pos = np.array([df[c].to_numpy() for c in test_rwd_cols if c in df])
data_neg = np.array([df[c].to_numpy() for c in test_cost_cols if c in df])
...
ax1.set_ylim(ymin=0, ymax=max(abs(ax1ymax), abs(ax2ymin)))
ax2.set_ylim(ymin=-max(abs(ax1ymax), abs(ax2ymin)), ymax=0)


plt.tight_layout()
plt.savefig(f"{seed}/plots/rewards-stacked.png", dpi=150)
